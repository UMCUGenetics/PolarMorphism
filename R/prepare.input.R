...
AlleleFlip <- function(sumstats, snps, snpid = "snpid"){
  ...
  sumstats$a1.b <- toupper(sumstats$a1.b); sumstats$a2.b <- toupper(sumstats$a2.b)
  if(identical(sumstats$a1, sumstats$a1.b)){
    sumstats <- sumstats %>%
      dplyr::select(-ends_with(".b")) %>%
      dplyr::mutate(z = beta/se)
  }else{
    same <- sumstats$a1 == sumstats$a1.b
    flip <- sumstats$a1 == sumstats$a2.b
    sumstats$freq <- ifelse(flip, 1 - sumstats$freq, sumstats$freq)
